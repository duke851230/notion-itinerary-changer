...
from helpers.excel.sheet import initialize_sheet, insert_activities_to_sheet, set_timeline_in_sheet
from helpers.excel.utils import get_timeline_data
from helpers.notion.utils import get_daily_activities
from openpyxl import Workbook
from openpyxl.worksheet.worksheet import Worksheet

FILE_PATH: str = os.path.join(configer.excel.EXCEL_BASIC_DIR, configer.excel.FILE_NAME)
...
def produce_schedule_excel(file_path: str) -> None:
    ...
    row_number_in_one_timeline_interval: int = (
        configer.excel.SCHEDULE_TIMELINE_INTERVAL // configer.excel.MINIMAL_INTERVAL
    )

    initialize_sheet(sheet=sheet, merge_row_num=row_number_in_one_timeline_interval)
    ...
    check_activities_time(
        activities,
        configer.excel.SCHEDULE_TIMELINE_START_AT,
        configer.excel.SCHEDULE_TIMELINE_END_AT,
    )
    ...
    insert_activities_to_sheet(
        sheet=sheet, activities=activities, timeline=end_timeline, start_column="B"
    )
